// RUCHI KOOTU - Secure Telemetry Admin Console
...
export default function AdminDashboard({ isOpen, onClose }) {
  ...
  const [feedback, setFeedback] = useState([
    { id: 1, name: "Rohit Sharma", email: "rohit@gmail.com", msg: "Loved the Royal Awadhi Biryani recommendation! The concierge is spot on.", status: "Pending" },
    { id: 2, name: "Priya Nair", email: "priya@outlook.com", msg: "Valet parking status for Kuro Shiro needs an update in the amenities listing.", status: "Pending" },
    { id: 3, name: "Vikram Dev", email: "vikram@dev.io", msg: "Outstanding interface. The Ruchi Score circular meter has a premium feel.", status: "Resolved" }
  ]);
  ...
  return html`
    <div className=${`fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md transition-opacity duration-500 ${isClosing ? 'opacity-0' : 'opacity-100'}`}>
      
      <!-- Close Overlay Backdrop -->
      <div className="absolute inset-0" onClick=${handleClose}></div>

      <!-- Main Admin Panel Container -->
      <div className=${`relative w-full max-w-5xl h-[90vh] bg-[#0A0A0A] border border-gold/20 rounded-2xl shadow-2xl flex flex-col overflow-hidden z-10 transition-transform duration-500 ease-in-out ${isClosing ? 'scale-95 opacity-0' : 'scale-100 opacity-100'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-8 flex flex-col gap-8 bg-black/20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="glass-panel p-6 rounded-xl border border-gold/10 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3.5 mt-2">
                ${[
                  { name: "Royal Awadhi", percentage: 35, count: 1687 },
                  { name: "Artisanal Japanese", percentage: 25, count: 1205 },
                  { name: "Modern South Indian", percentage: 20, count: 964 },
                  { name: "Contemporary French", percentage: 12, count: 578 },
                  { name: "New Nordic", percentage: 8, count: 387 }
                ].map(item => html`
                  <div key=${item.name} className="flex flex-col gap-1">
                    <div className="flex justify-between text-xs font-light text-neutral-300">
                      <span>${item.name}</span>
                      <span className="font-mono text-gold font-semibold">${item.percentage}% (${item.count})</span>
                    </div>
                    <div className="w-full h-1.5 bg-neutral-900 rounded-full overflow-hidden">
                      <div className="h-full bg-gold rounded-full" style=${{ width: `${item.percentage}%` }}></div>
                    </div>
                  </div>
                `)}
              </div>
            ...
            <div className="glass-panel p-6 rounded-xl border border-gold/10 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3 mt-2">
                ${[
                  { name: "Indiranagar quarters", count: 2024, density: "High Density", color: "text-rose-400 bg-rose-500/10 border-rose-500/20" },
                  { name: "Lavelle Road lounges", count: 1349, density: "High Density", color: "text-rose-400 bg-rose-500/10 border-rose-500/20" },
                  { name: "Koramangala central", count: 864, density: "Moderate Density", color: "text-amber-400 bg-amber-500/10 border-amber-500/20" },
                  { name: "Whitefield corridor", count: 584, density: "Light Density", color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/20" }
                ].map(loc => html`
                  <div key=${loc.name} className="flex items-center justify-between p-3 rounded-lg bg-[#121212]/30 border border-neutral-900">
                    <div className="flex flex-col min-w-0">
                      <span className="text-xs text-neutral-200 font-medium">${loc.name}</span>
                      <span className="text-[10px] text-neutral-500 font-mono mt-0.5">${loc.count} inspection points</span>
                    </div>
                    <span className=${`text-[9px] font-mono uppercase px-2 py-0.5 rounded border shrink-0 ${loc.color}`}>
                      ${loc.density}
                    </span>
                  </div>
                `)}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-5 glass-panel p-6 rounded-xl border border-gold/10 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3 mt-2">
                ${[
                  { name: "Kuro Shiro", ruchi: 96, views: 840, favs: 210 },
                  { name: "L'Ambroisie", ruchi: 92, views: 620, favs: 180 },
                  { name: "Swarna Mahal", ruchi: 89, views: 480, favs: 112 },
                  { name: "Noma Bangalore", ruchi: 78, views: 320, favs: 64 }
                ].map(rest => html`
                  <div key=${rest.name} className="flex justify-between items-center p-3 rounded-lg bg-[#121212]/30 border border-neutral-900">
                    <div className="flex flex-col">
                      <span className="text-xs text-neutral-200 font-medium font-serif">${rest.name}</span>
                      <span className="text-[9px] text-neutral-500 font-mono mt-0.5">${rest.views} views • ${rest.favs} favorites</span>
                    </div>
                    <div className="flex flex-col items-end">
                      <span className="text-xs font-mono font-bold text-gold">${rest.ruchi}</span>
            ...
            <div className="lg:col-span-7 glass-panel p-6 rounded-xl border border-gold/10 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3.5 mt-2">
                ${feedback.map(item => html`
                  <div key=${item.id} className="p-4 rounded-xl bg-[#121212]/40 border border-neutral-900/60 flex flex-col gap-2">
                    <div className="flex justify-between items-start">
                      <div className="flex flex-col">
                        <span className="text-xs text-neutral-300 font-medium">${item.name} (${item.email})</span>
                      </div>
                      <span className=${`text-[9px] font-mono uppercase px-2 py-0.5 rounded border ${item.status === 'Resolved' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : item.status === 'Escalated' ? 'bg-rose-500/10 text-rose-400 border-rose-500/20' : 'bg-amber-500/10 text-amber-400 border-amber-500/20'}`}>
                        ${item.status}
                      </span>
                    </div>
